...
import { getSlowMoversAPI } from "../../api/utils";
...
const useScreenHooks = (props) => {
    ...
    const fetchData = async (deps) => {
        try {
            ...
            const res = await getSlowMoversAPI(authToken, params);
            ...
            if (res && res?.data) {
                ...
                if (data?.responseStatus == '200') {
                    ...
                    ErrorToast('Slow Movers', msg);
                }
            } else {
                ErrorToast('Slow Movers', 'Something went wrong.');
            }
        } catch (error) {
            setLoading(false);
